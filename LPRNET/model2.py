...
class LPRNet_vgg():
    ...
    def _build(self):
        ...
        x = self.basic_block(x.get_shape().as_list()[3], 512)(x)
        x = self.convolution_block(x.get_shape().as_list()[3], 512, 2)(x)

        x = Dropout(self.dropout)(x)
        x = self.conv2D_batchnorm(512, [4, 1])(x)
        x = Dropout(self.dropout)(x)
        classes = self.conv2D_batchnorm(self.num_classes, [1, 13], padding="same")(x)
        pattern = Conv2D(128, [1,1])(classes)
        x = Concatenate()([classes, pattern])
        outs = self.conv2D_batchnorm(self.num_classes, [1, 1], padding="same")(x)

        return Model(inputs = inputs, outputs = outs)

    def conv2D_batchnorm(self, *args, **kwargs):
        return Sequential([Conv2D(*args, **kwargs),
                    BatchNormalization(),
                    ReLU()])
    
    def basic_blocks(self, channel_in, channel_out):
        return Sequential([self.conv2D_batchnorm(channel_out // 4, [1, 1], padding="same"),
                           self.conv2D_batchnorm(channel_out // 4, [3, 1], padding="same"),
                           self.conv2D_batchnorm(channel_out // 4, [1, 3], padding="same"),
                           self.conv2D_batchnorm(channel_out // 4, [3, 3], padding="same"),
                           self.conv2D_batchnorm(channel_out // 4, [1, 1], padding="same")
                           ])
    ...
    def mixed_input_block(self):
        return Sequential([self.conv2D_batchnorm(64, [3, 3], padding="same"),
